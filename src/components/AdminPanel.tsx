...
export function AdminPanel({ config, onChange }: AdminPanelProps) {
  ...
  const ColorInput = ({ label, name }: { label: string; name: keyof TemplateConfig }) => (
    <div className="flex items-center justify-between py-3 border-b border-[#F5F5F0] last:border-0">
      ...
      <div className="flex items-center gap-3">
        <span className="text-[10px] font-mono text-gray-400 uppercase w-16 text-right">{config[name]}</span>
        <div className="relative w-7 h-7 rounded-full overflow-hidden border-2 border-white shadow-sm ring-1 ring-[#E2E2D8]">
          <input
            type="color"
            name={name}
            value={config[name]}
  ...
  return (
    <div className="bg-white rounded-2xl shadow-xl shadow-black/5 flex flex-col border border-white/50 w-full overflow-hidden mb-12">
      <div className="p-6 bg-[#F5F5F0] border-b border-[#E2E2D8]">
        <h2 className="font-sans italic text-xl text-[#5A5A40] mb-1">Cài đặt hiển thị</h2>
      ...
      <div className="p-8 space-y-8">
        ...
        <div>
          ...
          <div className="pl-4 border-l border-[#E2E2D8] ml-2">
            <ColorInput label="Màu chữ nội dung" name="textColor" />
            <ColorInput label="Màu Tiêu đề chính" name="headingColor1" />
            <ColorInput label="Màu Tiêu đề phụ" name="headingColor2" />
          </div>
        </div>

        <div>
          <h3 className="text-[10px] font-black text-[#5A5A40] uppercase tracking-widest mb-4">Màu sắc khung viền</h3>
          <div className="pl-4 border-l border-[#E2E2D8] ml-2">
            <ColorInput label="Viền cột trái" name="borderColor1" />
            <ColorInput label="Viền cột phải" name="borderColor2" />
            <ColorInput label="Viền bảng dưới" name="borderColor3" />
          </div>
        </div>
      </div>
    </div>
